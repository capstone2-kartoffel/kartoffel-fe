...
// 입력된 문자열을 Date 객체로 파싱
    ...
    const day = String(date.getDate()).padStart(2, '0'); // 날짜를 두 자리로 패딩
     return `${year}-${month}-${day}`;
}
...
document.getElementById('profile-upload').addEventListener('change', function(event) {
    ...
    reader.onload = function() {
        ...
        img.src = reader.result; // Update the profile picture with the selected image
        
        //수정
        // Store the URL of the selected photo in local storage
        localStorage.setItem('profilePhotoURL', reader.result);
    }

    if (file) {
        reader.readAsDataURL(file); // Convert the selected file to a data URL
        document.getElementById('selected-file-name').textContent = file.name; // Display the selected file name
    } else {
        document.getElementById('selected-file-name').textContent = ''; // Clear the file name if no file is selected
    }
});
